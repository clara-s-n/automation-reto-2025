    ...
    public void loginCredencialesIncorrectas() {
        try {
            ...
            boolean loginFallo = driver.getCurrentUrl().contains("login") ||
                                isElementPresent(By.xpath("//*[contains(text(),'error') or contains(text(),'Error') or contains(text(),'incorrecto') or contains(text(),'inválido')]")) ||
    ...
    public void restriccionAccesoUsuarioNormal() {
        try {
            ...
            boolean accesoRestringido = 
                !currentUrl.contains("administracion") ||
